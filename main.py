...
class AutoUpdater:
    ...
    def scrape_policy(self):
        ...
        try:
            # Policy mentése "store_policy" ID-val
            # A timestamp itt nem számít, mert ezt nem töröljük a takarításnál
    ...
    def update_books_from_feed(self):
        ...
        # 1. Jelenlegi időbélyeg (timestamp)
        # Ezt fogjuk rátenni minden megtalált könyvre.
        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    # Mivel a feedben CSAK készleten lévő van, ezért fixen instock
                    ...
                    metadata = {
                        ...
                        "last_seen": current_sync_ts # <--- A BÉLYEGZŐ!
                    }
            ...
            # 2. TAKARÍTÁS (GARBAGE COLLECTION)
            # Törlünk mindent, aminek a 'last_seen' értéke kisebb, mint a mostani indítás ideje.
            # VAGY aminek nincs 'last_seen' értéke (régi adatok).
            # A 'store_policy' rekordot KIVÉTELBE tesszük (bár annak nincs last_seen-je, a filter ezt megoldja)
...
class BooksyBrain:
    ...
    def search_engine_logic(self, query_text, lang_filter):
        try:
            ...
            # CSAK INSTOCK (Bár elvileg már csak azok vannak az indexben)
            filt = {"stock": "instock"}
...
@app.get("/")
def home(): return {"status": "Booksy V26 (Auto Mirror Sync Active)"}
